...
const Statistics = () => {
  ...
  const generateJobReport = async () => {
    try {
      ...
      const totalPlanProdTime = yearJobs.reduce((sum, job) => sum + (Number(job.planprodtime) || 0), 0)
      
      statsRow.getCell(1).value = `Total Jobs: ${totalJobs} | Total Order: ${totalOrder.toLocaleString()} | Total Duration: ${totalDuration.toFixed(1)} hrs | Total Plan Prod Time: ${totalPlanProdTime.toFixed(1)} mins`
      ...
      const headers = [
        ...
        'Downtime', 'Operating Time', 'Plan Prod Time', 'Prod Lead Time', 'Availability',
      ...
      if (yearJobs.length === 0) {
        ...
        worksheet.mergeCells(`A${rowIndex}:Q${rowIndex}`)
      ...
      if (yearJobs.length > 0) {
        ...
        worksheet.mergeCells(`A${rowIndex + 1}:Q${rowIndex + 1}`)
      }
  ...
  const StatsCards = () => {
    ...
    const totalPlanProdTime = yearlyData.reduce((sum, job) => sum + (Number(job.planprodtime) || 0), 0)
    ...
    return (
      <div className="mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="text-center p-4 bg-sky-50 dark:bg-sky-900 rounded-lg shadow">
            <div className="text-3xl font-bold text-sky-600 dark:text-sky-400">
              {totalPlanProdTime.toFixed(1)}
            </div>
            <div className="text-sm text-sky-600 dark:text-sky-400 font-medium">Plan Prod Time</div>
            <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              Avg: {totalJobs > 0 ? (totalPlanProdTime / totalJobs).toFixed(1) : 0} mins/job
            </div>
          </div>
        </div>
          ...
          <div className="text-center p-4 bg-blue-50 dark:bg-blue-900 rounded-lg shadow">
            ...
            <div className="mt-3 bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
                ...
                style={{ width: `${Math.min(avgAvailability * 100, 100)}%` }}
              />
            </div>
          </div>
          ...
          <div className="text-center p-4 bg-green-50 dark:bg-green-900 rounded-lg shadow">
            ...
            <div className="mt-3 bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
                ...
                style={{ width: `${Math.min(avgPerformance * 100, 100)}%` }}
              />
            </div>
          </div>
          ...
          <div className="text-center p-4 bg-purple-50 dark:bg-purple-900 rounded-lg shadow">
            ...
            <div className="mt-3 bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
                ...
                style={{ width: `${Math.min(avgQuality * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
          ...
          <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900 rounded-lg shadow">
            ...
            <div className="mt-3 bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
                ...
                style={{ width: `${Math.min(avgOEE * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
          ...
          <div className={`space-y-4 rounded-lg p-1 ${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`}>
            <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div><span className="font-medium">Plan Prod Time:</span> {selectedJob.planprodtime || 0} mins</div>
                </div>
              </div>
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4 text-sm`}>
                ...
                <div>
                  <span className="font-medium">Plan Prod Time:</span> {selectedJob.planprodtime || 0} mins
                </div>
                <div>
