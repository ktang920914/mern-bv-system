...
const generateExcelReport = async () => {
    try {
        ...
        // 数据行
        let rowIndex = 4  // 注意：从第4行开始，因为第1行是标题，第2行是副标题，第3行是表头
        let totalQuantity = 0
        let totalAmount = 0
        ...
        excelData.forEach((order, index) => {
            ...
            rowData.forEach((value, colIndex) => {
                ...
                // 不同的列对齐方式
                if (colIndex === 0 || colIndex === 6 || colIndex === 7) {
        ...
        if (excelData.length > 0) {
            const autoFilterRange = {
                ...
                to: { row: rowIndex - 1, column: 12 }  // 到最后一行
            };
        ...
        totalRow.getCell(6).value = totalQuantity
        totalRow.getCell(6).font = boldFont
        totalRow.getCell(6).alignment = rightAlignment
        ...
        worksheet.mergeCells(`F${rowIndex}:L${rowIndex}`)
        ...
        // 使用日期和时间作为文件名，格式与 OEE 组件相同
        return { blob, fileName: `Orders_Report_${dateStr}_${timeStr}.xlsx` }
