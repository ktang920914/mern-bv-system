import { TextInput, Button, Table, TableHead, TableHeadCell, TableRow, Modal, ModalHeader, ModalBody, Label, Select, Spinner, Alert, TableBody, TableCell, Pagination, ModalFooter } from "flowbite-react";
...
const Movement = () => {
    ...
    <div className={`p-4 mb-4 rounded-lg shadow transition-all duration-200 ${
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div>
          ...
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.date}</p>
        </div>
        <div>
          <p className="text-sm font-semibold text-gray-500">Transaction</p>
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.transaction}</p>
        </div>
        <div>
          <p className="text-sm font-semibold text-gray-500">Quantity</p>
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.quantity}</p>
        </div>
        <div>
          <p className="text-sm font-semibold text-gray-500">Balance</p>
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.balance}</p>
        </div>
      </div>
      
      <div className="mb-3">
        <p className="text-sm font-semibold text-gray-500">Stock Item</p>
        <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.item}</p>
      </div>

      <div className="grid grid-cols-2 gap-2 mb-3">
        <div>
          <p className="text-sm font-semibold text-gray-500">User</p>
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.user}</p>
        </div>
        <div>
          <p className="text-sm font-semibold text-gray-500">Status</p>
          <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{movement.status}</p>
        </div>
      </div>
      ...
      {currentUser.role === 'Admin' && (
        <div className="flex gap-2">
          ...
          <Button 
            ...
            className='cursor-pointer flex-1 py-2 text-sm transition-all hover:scale-105' 
            onClick={() => {
              setMovementIdToDelete(movement._id)
  ...
  // 修改后的 generateExcelReport 函数 - 类似 Transaction.jsx 版本
  const generateExcelReport = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet(`Stock Movements Report ${dateStr}`)
      ...
      titleRow.height = 30
      titleRow.getCell(1).value = 'STOCK MOVEMENTS REPORT'
      ...
      headerRow.height = 25
      const headers = [
        'No.', 'Date', 'Stock Item', 'Transaction Type', 'Quantity', 
      ...
      const excelData = movements.map(movement => ({
        ...
        'Created At': new Date(movement.createdAt).toLocaleString('en-US', {
          year: 'numeric',
          month: '2-digit',
          day: '2-digit',
          hour: '2-digit',
        ...
        'Updated At': new Date(movement.updatedAt).toLocaleString('en-US', {
          year: 'numeric',
          month: '2-digit',
          day: '2-digit',
          hour: '2-digit',
      ...
      excelData.forEach((movement, index) => {
      ...
      if (excelData.length === 0) {
        const row = worksheet.getRow(rowIndex)
        row.getCell(1).value = 'No stock movement data available'
      ...
      // 生成带时间戳的文件名
      const fileName = `Stock_Movements_Report_${dateStr}_${timeStr}.xlsx`
    ...
    <div className="min-h-screen">
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4'>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <Button className='cursor-pointer flex-1 sm:flex-none' onClick={handleCreateMovement}>
            Create movement
          </Button>
          <Button 
