...
const Spareparts = () => {
    ...
    const [openModalCreateSparepart, setOpenModalCreateSparepart] = useState(false);
    const [openModalDeleteSparepart, setOpenModalDeleteSparepart] = useState(false);
    const [openModalUpdateSparepart, setOpenModalUpdateSparepart] = useState(false);
    const [errorMessage, setErrorMessage] = useState(null);
    const [loading, setLoading] = useState(false);
    const [suppliers, setSuppliers] = useState([]);
    const [spareparts, setSpareparts] = useState([]);
    const [sparepartIdToDelete, setSparepartIdToDelete] = useState('');
    const [sparepartIdToUpdate, setSparepartIdToUpdate] = useState('');
    ...
    // Open/close modals
    const handleCreateSparepart = () => { setOpenModalCreateSparepart(!openModalCreateSparepart); setErrorMessage(null); setLoading(false); };
    const handleUpdate = (sparepart) => {
        setErrorMessage(null); setLoading(false);
        setOpenModalUpdateSparepart(true);
        setSparepartIdToUpdate(sparepart._id);
        setUpdateFormData({
            code: sparepart.code,
            type: sparepart.type,
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await fetch('/api/other/sparepart', {
                method: 'POST',
            ...
            if(data.success === false) setErrorMessage(data.message);
                ...
                setOpenModalCreateSparepart(false);
                const sparepartsRes = await fetch('/api/other/getSpareparts');
                const sparepartsData = await sparepartsRes.json();
                if(sparepartsRes.ok) setSpareparts(sparepartsData);
            }
        } catch (error) {
            console.log(error.message);
        } finally {
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/other/update/${sparepartIdToUpdate}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if(data.success === false) setErrorMessage(data.message);
                ...
                setOpenModalUpdateSparepart(false);
                const sparepartsRes = await fetch('/api/other/getSpareparts');
                const sparepartsData = await sparepartsRes.json();
                if(sparepartsRes.ok) setSpareparts(sparepartsData);
            }
    ...
    const handleDelete = async () => {
        setOpenModalDeleteSparepart(false);
        try {
            const res = await fetch(`/api/other/delete/${sparepartIdToDelete}`, { method: 'DELETE' });
            const data = await res.json();
            if(res.ok) setSpareparts(prev => prev.filter(sparepart => sparepart._id !== sparepartIdToDelete));
        } catch (error) {
            console.log(error.message);
        }
    };
    ...
    const filteredSpareparts = spareparts.filter(sparepart =>
        sparepart.code.toLowerCase().includes(searchTerm) && sparepart.code.toLowerCase() === searchTerm ||
        sparepart.type.toLowerCase().includes(searchTerm) ||
        sparepart.location.toLowerCase().includes(searchTerm) ||
        sparepart.supplier.toLowerCase().includes(searchTerm) ||
        sparepart.status.toLowerCase().includes(searchTerm) ||
        sparepart.balance.toString().toLowerCase().includes(searchTerm)
    );
    const indexOfLastItem = currentPage * itemsPage;
    const indexOfFirstItem = indexOfLastItem - itemsPage;
    const currentSpareparts = filteredSpareparts.slice(indexOfFirstItem, indexOfLastItem);
    const totalEntries = filteredSpareparts.length;
    ...
    const generateQRContent = (sparepart) => {
        // 优先使用后端存储的 QR 码内容
        if (sparepart && sparepart.qrCode) {
            return sparepart.qrCode;
        }
        ...
        return JSON.stringify({
            ...
            supplier: sparepart?.supplier || '',
            status: sparepart?.status || '',
            balance: sparepart?.balance !== undefined ? sparepart.balance : 0,
            createdAt: sparepart?.createdAt || new Date().toISOString(),
        ...
        <div className={`p-4 mb-4 rounded-lg shadow transition-all duration-200 ${
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
                    ...
                    <Popover 
                        ...
                        content={
                            <div className="p-4 text-center">
                                <h3 className="font-semibold mb-2">QR Code - {sparepart.code}</h3>
                                <QRCodeCanvas value={generateQRContent(sparepart)} size={150} level="M" includeMargin={true}/>
                                <p className="text-xs dark:text-gray-300 text-gray-500 mt-2">Scan to view sparepart details</p>
                            </div>
                        }
                        trigger='hover'
                        placement="top"
                ...
                <div>
                    ...
                    <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{sparepart.type}</p>
                </div>
                <div>
                    <p className="text-sm font-semibold text-gray-500">Location</p>
                    <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{sparepart.location}</p>
                </div>
                <div>
                    <p className="text-sm font-semibold text-gray-500">Status</p>
                    <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{sparepart.status}</p>
                </div>
            </div>
            
            <div className="mb-3">
                <p className="text-sm font-semibold text-gray-500">Supplier</p>
                <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{sparepart.supplier}</p>
            </div>

            <div className="mb-3">
                <p className="text-sm font-semibold text-gray-500">Balance</p>
                <p className={`${theme === 'light' ? 'text-gray-900' : 'text-gray-100'}`}>{sparepart.balance}</p>
            </div>
            ...
            <div className="flex gap-2">
                <Button 
                    ...
                    onClick={() => handleUpdate(sparepart)}
                >
                    Edit
                </Button>
                {currentUser.role === 'Admin' && (
                    <Button 
                        ...
                        className='cursor-pointer flex-1 py-2 text-sm transition-all hover:scale-105' 
                        onClick={() => {
                            setSparepartIdToDelete(sparepart._id)
                            setOpenModalDeleteSparepart(true)
                        }}
                    >
                        Delete
                    </Button>
    ...
    const generateExcelReport = async () => {
      try {
        ...
        const worksheet = workbook.addWorksheet(`Spareparts Report ${dateStr}`)
        ...
        // 生成带时间戳的文件名
        const fileName = `Spareparts_Report_${dateStr}_${timeStr}.xlsx`
    ...
    return (
        <div className='min-h-screen'>
            <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4'>
                <h1 className='text-2xl font-semibold'>Spareparts</h1>
                ...
                <div className='flex gap-2 w-full sm:w-auto'>
                    <Button className='cursor-pointer flex-1 sm:flex-none' onClick={handleCreateSparepart}>
                        Create Sparepart
                    </Button>
                    <Button 
                ...
                <Table hoverable className="[&_td]:py-1 [&_th]:py-2">
                    ...
                    <TableBody>
                        {currentSpareparts.map(sparepart => (
                            <TableRow key={sparepart._id} className={`${theme === 'light' ? ' text-gray-900 hover:bg-gray-300' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
                                <TableCell>
                                    <Popover className={`${theme === 'light' ? ' text-gray-900 bg-gray-200 hover:bg-gray-100' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                                        ...
                                        arrow={false}
                                    >
                                        <span className="cursor-pointer hover:text-blue-600 transition-colors border-b border-dashed border-blue-300">{sparepart.code}</span>
                                    </Popover>
                                </TableCell>
                                <TableCell>{sparepart.type}</TableCell>
                                <TableCell>{sparepart.location}</TableCell>
                                <TableCell>{sparepart.supplier}</TableCell>
                                <TableCell>{sparepart.status}</TableCell>
                                <TableCell>{sparepart.balance}</TableCell>
                                <TableCell><Button className='cursor-pointer py-1 px-1 text-sm h-8' outline onClick={() => handleUpdate(sparepart)}>Edit</Button></TableCell>
                                {currentUser.role === 'Admin' && (
                                <TableCell><Button className='cursor-pointer py-1 px-1 text-sm h-8' color='red' outline onClick={() => { setSparepartIdToDelete(sparepart._id); setOpenModalDeleteSparepart(true); }}>Delete</Button></TableCell>
                                )}
                            </TableRow>
                        ))}
                    </TableBody>
            ...
            {/* 模态框保持不变 */}
            <Modal show={openModalCreateSparepart} onClose={handleCreateSparepart} popup>
                <ModalHeader className={`${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`} />
                <ModalBody className={`${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`}>
                    <div className="space-y-6">
                        <h3 className={`text-xl font-medium ${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`}>Create Sparepart</h3>
            ...
            <Modal show={openModalUpdateSparepart} onClose={() => setOpenModalUpdateSparepart(false)} popup>
                <ModalHeader className={`${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`} />
                <ModalBody className={`${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`}>
                    <div className="space-y-6">
                        <h3 className={`font-medium text-xl ${theme === 'light' ? '' : 'bg-gray-900 text-gray-50'}`}>Update Sparepart</h3>
                        
                        {/* 添加 QR 码显示区域 */}
                        <div className="flex justify-center mb-4">
                            <div className="text-center">
                                <QRCodeCanvas className='text-center'
                                    value={generateQRContent(spareparts.find(sparepart=> sparepart._id === sparepartIdToUpdate) || {})} 
                                    size={120} 
                                    level="M" 
                                    includeMargin={true}
                                />
            ...
            <Modal show={openModalDeleteSparepart} size="md" onClose={() => setOpenModalDeleteSparepart(false)} popup>
                <ModalHeader />
                <ModalBody>
                    <div className="text-center">
